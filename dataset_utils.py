...
from torch_geometric.datasets import LINKXDataset, Flickr
...
def DataLoader(name, normalize_data):
    ...
    if name in ['cora', 'citeseer', 'pubmed']:
        ...
        valid_mask = (data.y != -1)
        valid_mask_reshaped = valid_mask.unsqueeze(1)
        data.train_mask = data.train_mask & valid_mask_reshaped
        data.val_mask = data.val_mask & valid_mask_reshaped
        data.test_mask = data.test_mask & valid_mask_reshaped
        dataset.data = data
        ...
        if normalize_data:
         ...
         transform2 = RandomNodeSplit(split="train_rest", num_splits=10, num_test=0.2, num_val=0.32)
         data = transform2(dataset[0].clone())
         dataset.data = data
    elif name == 'flickr':
        path = osp.join('../data_v2', name)
        dataset = Flickr(path)
    else:
